...
| 改 Pulse 根界面（Hero/Featured/Supporting/Noticed/Entry/sheets） | modules/pulse-stream.md |
| 改聊天界面（Talk header/气泡/输入区） | modules/chat.md |
...
当前形态（Companion UI Migration，2026-09-21）：Expo（SDK 57）应用；**单表面 IA**——Pulse 是唯一根界面（无 tab bar），
...
| Settings sheet（连接与账号 / BFF 地址 / model 偏好 + model picker） | pulse-stream | `agent-mobile-app/src/components/pulse/SettingsSheet.tsx` |
...
| Memory sheet（REMEMBERS 记忆 + forget） | pulse-stream | `agent-mobile-app/src/components/pulse/MemorySheet.tsx` + `services/memory/` |
| Knowledge sheet（KB 检索 → `/kb/doc`） | pulse-stream | `agent-mobile-app/src/components/pulse/KnowledgeSheet.tsx` |
| KB 文档阅读屏（`/kb/doc`） | services | `agent-mobile-app/src/app/kb/doc.tsx` |
| Runtime presence（opencode 离线识别与诚实呈现） | services | `agent-mobile-app/src/services/runtime-presence.ts` |
| Responsibilities / Assignment 管理面 + 详情（授权/执行历史投影，revoke/repair/compensate） | services | `agent-mobile-app/src/app/assignments/` + `services/assignment/` |
| Attention 详情屏（evidence + 关联 assignment 只读投影） | services | `agent-mobile-app/src/app/attention/[id].tsx` |
| L1 订阅（market + observation：Supporting MARKET 行 + Noticed） | services | `agent-mobile-app/src/services/l1.ts` + `hooks/useL1.ts` |
| Suggestion 数据面（proposal 只读投影 + product SSE + confirm/reject 转发） | services | `agent-mobile-app/src/services/proposal/` + `hooks/useSuggestions.ts` |
| 对话面板（下拉刷新/分页/滚动保持/输入/轮询兜底/agent+model切换/question+permission弹窗） | chat | `agent-mobile-app/src/components/chat/ChatPanel.tsx` |
| 消息气泡（user/text markdown + 错误 + StepChip 旁白） | chat | `agent-mobile-app/src/components/chat/MessageBubble.tsx` |
...
| 路由 | expo-router | ~57.0.11 | 文件路由（单表面 Stack，无 Tabs） |
...
└── agent-mobile-app/                          # ★ 当前活跃的 Expo 应用
    ...
    ├── scripts/
    │   ├── serve-static.mjs                   # 静态服务器（gzip；端口硬编码 9928，含 / 与 [id].html 回退）
    │   ├── e2e/pulse-e2e.mjs                  # Playwright E2E（含发消息）
    │   └── e2e/phase13-e2e.mjs                # Playwright E2E（Phase 13 链路）
    │   └── reset-project.js                   # create-expo-app 模板残留（可忽略）
    ├── assets/                                # 图标/启动图
    └── src/
        ├── app/_layout.tsx                    # 根 Stack（单表面：index/talk/assignments/attention/kb）
        ├── app/index.tsx                      # Pulse 根界面（Hero/Featured/Supporting/Noticed/Entry）
        ├── app/talk.tsx                       # Talk contextual stack
        ├── app/assignments/                   # Responsibilities 列表 + 详情
        ├── app/attention/[id].tsx             # Attention 详情
        ├── app/kb/doc.tsx                     # KB 文档阅读
        ├── app/memory.tsx / me.tsx            # legacy Redirect → /
        ├── config/opencode.ts                 # opencode 连接配置（env）
        ├── hooks/useProjectEvents.ts          # 项目事件聚合 hook
        ├── services/                          # ★ opencode REST/SSE/reducer/step展开/状态机
        ├── components/                        # primitives/feedback/navigation/chat/pulse + index.ts
...
theme（无依赖，叶子）
  ↑
components（依赖 theme；含 pulse/ Companion 视觉层）
  ↑
services（opencode-client/events/reducer/merging/project-status/l1；无 UI 依赖）
  ↑
chat（ChatPanel/MessageBubble/StepChip/ProjectChatZ；依赖 components + services + theme）
  ↑
pulse-stream（index.tsx + hooks；依赖 components + chat + services + theme）
  ↑
router（app/_layout → index/talk/assignments/attention/kb，依赖全部）
  ↑
...
opencode server (127.0.0.1:4096, Basic auth)
  ...
  │     ├── → useProjectEvents → index.tsx（RUNNING 行）
  │     └── → ChatPanelZ → message-reducer（增量 patch）→ message-merging → MessageBubbleZ
  └── 发送消息：ChatPanelZ → sendMessageAsync (prompt_async) → SSE 回流

family-finance BFF (106.13.181.13:19234)，JWT，全部为 projection（canonical 在 BFF SQLite / memx / vault）：
  ├── /api/product/attention（snapshot）+ /api/product/stream（SSE：attention.* + proposal.*）→ services/attention → useAttentions → Featured + Supporting(NEEDS YOU)
  ├── /api/product/assignment-proposals（snapshot ?status=proposed，SSE proposal.*）→ services/proposal → useSuggestions → Supporting(SUGGESTED)
  ├── /api/product/l1 + /api/product/l1/stream（SSE 全帧）→ services/l1 → useL1 → Supporting(MARKET) + Noticed
  ├── /api/product/assignments(+detail/history/repair/compensate/revoke) → services/assignment → /assignments + FundSheet
  └── /api/product/memory + /api/product/kb/* → services/memory → MemorySheet / KnowledgeSheet
```
...
| 改 Pulse 界面（Hero/Featured/Supporting/Noticed/Entry/sheets） | `agent-mobile-app/src/app/index.tsx` + `src/components/pulse/` |
...
| 改 ZCode 风格聊天（header orb/气泡/输入区） | `agent-mobile-app/src/components/chat/zcode/`（fork 自 ChatPanel，双维护见 chat.md） |
...
| 改 model 偏好 / Settings | `agent-mobile-app/src/components/pulse/SettingsSheet.tsx` + `services/model-prefs.ts`；ChatPanelZ 默认 pill 优先级 Settings 偏好 > server agent.model > FALLBACK_AGENTS |
| 改后端地址/账号 | `agent-mobile-app/.env.local`（EXPO_PUBLIC_OPENCODE_*） |
| 新增页面（ contextual stack route） | `agent-mobile-app/src/app/<name>.tsx` + 在 `_layout.tsx` 登记 `Stack.Screen` |
| 新增 contextual sheet | `agent-mobile-app/src/components/pulse/<X>Sheet.tsx` + 在 `index.tsx` 挂载 |
