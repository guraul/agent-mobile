...
- [ ] **Step 1: 改 listAgents useEffect(395-414)**
...
- [ ] **Step 2: 改初始 model pill(417-444 的 session adopt useEffect 后加偏好优先)**
...
- [ ] `pnpm test` 全过(新增 bff-config/auth/filter-models/model-prefs/bff-health 测试)
- [ ] Me 页三 Card 渲染正常(连接与账号/BFF 地址/model 偏好)
- [ ] model 选择 BottomSheet:SearchInput 过滤 + ScrollView 滚动
- [ ] 改 BFF 地址 → 提示重启生效 → 刷新后 `getBaseUrl()` 用新地址
- [ ] ChatPanel 默认 pill 读 Me 偏好(优先 server > fallback)
- [ ] 登出:token/username 清,回未登录态
