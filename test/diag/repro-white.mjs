...
const state = await page.evaluate(() => {
  ...
  return {
    ...
    bodyTextHead: body.innerText.slice(0, 200),
    scrollHeight: html.scrollHeight,
    clientHeight: html.clientHeight,
    bodyBg: getComputedStyle(body).backgroundColor,
    htmlBg: getComputedStyle(html).backgroundColor,
    hasSheet: !!document.querySelector('[data-testid="project-chat-sheet"]'),
    sheetChildren: document.querySelector('[data-testid="project-chat-sheet"]')?.children.length ?? 0,
  };
});
console.log('state:', JSON.stringify(state, null, 2));
console.log('--- logs ---');
console.log(logs.slice(-40).join('\n'));
await page.screenshot({ path: '/root/project/agent-mobile/test/white-repro.png' });
await browser.close();
